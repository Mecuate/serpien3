...
(function(mod) {
  ...
  CodeMirror.defineExtension("showHint", function(options) {
    ...
    if (this.somethingSelected()) {
      ...
      for (const selection of selections)
        if (selection.head.line !== selection.anchor.line) return;
    }
  ...
  Completion.prototype = {
    ...
    pick: function(data, i) {
      let completion = data.list[i], self = this;
  ...
  function Widget(completion, data) {
    this.id = "cm-complete-code_editor_widget_id"
    ...
    for (let i = 0; i < completions.length; ++i) {
      let elt = hints.appendChild(ownerDocument.createElement("li")), cur = completions[i];
    ...
    let scrolls = completion.options.paddingForScrollbar ? hints.scrollHeight > hints.clientHeight + 1 : false;
    ...
    if (overlapY > 0) { // Does not fit below
      let height = box.bottom - box.top, spaceAbove = box.top - (pos.bottom - pos.top) - 2
    ...
    let selectedHintRange = this.getSelectedHintRange();
  ...
  Widget.prototype = {
    close: function() {
      ...
      let input = this.completion.cm.getInputField()
    ...
    scrollToActive: function() {
      let selectedHintRange = this.getSelectedHintRange();
      let node1 = this.hints.childNodes[selectedHintRange.from];
      let node2 = this.hints.childNodes[selectedHintRange.to];
      let firstNode = this.hints.firstChild;
    ...
    getSelectedHintRange: function() {
      let margin = this.completion.options.scrollMargin || 0;
      return {
